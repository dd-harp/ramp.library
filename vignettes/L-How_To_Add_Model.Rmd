...
`LBionomics.newLname` computes the current bionomic parameter values by calling the forcing function for each parameter. It also resets all effect sizes to 1.

+ Replace `p1`, `p2`, ... with the names of the bionomic parameters in your model. Add a call to a forcing function for each one.

+ For each parameter `p1`, add a matching effect size reset: `es_p1 <- rep(1, nHabitats)`.
...
`LEffectSizes.newLname` applies vector control effect sizes to the bionomic parameters set by `LBionomics`. For each parameter, the effective value is the baseline times the effect size.

+ For each parameter `p1` with baseline `p1_t` and effect size `es_p1`, add a line: `xds_obj$L_obj[[s]]$p1 <- p1_t * es_p1`.
