...
To configure a new model, you must choose a string called `newXname`. The only rule for choosing `newXname` is that it must be unique. No other ${\cal XH}$ class model in `ramp.xde` or `ramp.library` can be using that string already. All the `S3` functions get dispatched by the string that replaces `newXname.` 

The 17 required functions are:

**Derivatives** 

1. `dXdt.newXname` - differential equations are defined by a system of differential equations. in `ramp.xde` these are encoded in a function called `dXdt` that  computes and returns the **derivatives.**  The function is set up so that the system can be solved by `deSolve::ode` or `deSolve::dede.` 
...
2. `make_Xinits_newXname` - each model requires a function that documents the variables by name and assigns initial values 
    
3. `setup_Xinits.newXname` - is a wrapper, that gets called by `ramp.xde::xde_setup.` It gets called by `make_Xinits_newXname,` and for the $i^{th}$ host, the initial values are stored as `pars$Xinits[[i]].` 
...
5. `list_Xvars.newXname` - retrieves the value of variables at a point in time and returns the values by name in a list; the function gets called by `dXdt` and by `update_inits_X` and it can be useful in other contexts. 

6. `get_inits_X.newXname` - retrieves the stored initial values in the same order that they are returned by `dXdt.newXname`

7. `update_inits_X.newXname` - a utility that makes it possible to extract and store a new set of initial values for `newXname` from a vector $y$ holding all the variables in a model. 

**Parameters**

8. `make_Xpar_newXname` returns a formatted list: 

    + the parameter values are stored by name 
   
    + `class(pars)` = `newXname` 
    
9. `setup_Xpar.newXname` is a wrapper that calls `make_Xpar_newXname` 
...
16. `xde_lines_X.newXname` 

17. `xde_plot_X.newXname` is a wrapper that calls `xde_lines_X` 
...
In the `xde_setup` family of setup functions, initial values for a model fulfilling the dynamical component $\cal X,$ called `newXname` is set up by the function `setup_Xinits.` 
...
In the `xde_setup` family of setup functions, setup for a model fulfilling the dynamical component $\cal X,$ called `newXname` is set up by the function `setup_Xpar.` 
...
The following is a template for new model. The declaration of `inheritParams` from the `SIS` model in `ramp.xde` works naturally. The namespace from `ramp.xde` is needed, so our newXname function (above) has `ramp.xde::F_X.` 
