...
3. `setup_Xinits.newXname` - is a wrapper, that gets called by `ramp.xds::xde_setup.` It gets called by `make_Xinits_newXname,` and for the $i^{th}$ host, the initial values are stored as `pars$Xinits[[i]].` 
