...
We have written this vignette to produce a new template file using `knitr::purl,` which can be modified to build a new model. Simply execute the following command, to create `Xmod-Xname.R.`  Then, rename `Xmod-Xname.R` by replacing `Xname` in the filename. Then open the file and do a global search for `Xname` and replace it with your new name. 
...
To configure a new model, you must choose a string called `Xname`. The only rule for choosing `Xname` is that it must be unique. No other ${\cal XH}$ class model in `ramp.xde` or `ramp.library` can be using that string already. All the `S3` functions get dispatched by the string that replaces `Xname.` 

The 17 required functions are:

**Derivatives** 

1. `dXdt.Xname` - differential equations are defined by a system of differential equations. in `ramp.xde` these are encoded in a function called `dXdt` that  computes and returns the **derivatives.**  The function is set up so that the system can be solved by `deSolve::ode` or `deSolve::dede.` 
...
2. `make_Xinits_Xname` - each model requires a function that documents the variables by name and assigns initial values 
    
3. `setup_Xinits.Xname` - is a wrapper, that gets called by `ramp.xde::xde_setup.` It gets called by `make_Xinits_Xname,` and for the $i^{th}$ host, the initial values are stored as `pars$Xinits[[i]].` 

4. `make_indices_X.Xname` - is the function that assigns an index to each variable in the model, and stores it as `pars$ix$X[[i]].` The indices are returned as a named list.  

5. `list_Xvars.Xname` - retrieves the value of variables at a point in time and returns the values by name in a list; the function gets called by `dXdt` and by `update_inits_X` and it can be useful in other contexts. 

6. `get_inits_X.Xname` - retrieves the stored initial values in the same order that they are returned by `dXdt.Xname`

7. `update_inits_X.Xname` - a utility that makes it possible to extract and store a new set of initial values for `Xname` from a vector $y$ holding all the variables in a model. 

**Parameters**

8. `make_Xpar_Xname` returns a formatted list: 

    + the parameter values are stored by name 
   
    + `class(pars)` = `Xname` 
    
9. `setup_Xpar.Xname` is a wrapper that calls `make_Xpar_Xname` 

**Dynamical Terms** are Computed using a Standard Set of Functions 

10. `F_X.Xname` - compute the effective infective density of the vertebrate hosts 

11. `F_H.Xname` - compute the human population density for the host strata

12. `F_b.Xname` - compute the probability a most will become infected after receiving an infective bite, which combines elements of this $\cal X$ model with a model of environmental heterogeneity in `Exposure.` 

**Outputs**

13. `parse_deout_X.Xname` - this function parses the output of `deSolve` and returns a named list that is either a vector or a matrix holding the values of the variables at each point in time. It works like `list_Xvars,` which accepts a vector holding $y$, the values of the variables at a single point in time. 

14. `F_pr.Xname` - this function takes 

15. `HTC.Xname`

**Outputs**

16. `xde_lines_X.Xname` 

17. `xde_plot_X.Xname` is a wrapper that calls `xde_lines_X` 
...
In the `xde_setup` family of setup functions, initial values for a model fulfilling the dynamical component $\cal X,$ called `Xname` is set up by the function `setup_Xinits.` 
...
To format this for a new function, do a search and replace on `Xname` and replace the parmeter names and values $p1$, $p2$, $\ldots$ 
...
In the `xde_setup` family of setup functions, setup for a model fulfilling the dynamical component $\cal X,$ called `Xname` is set up by the function `setup_Xpar.` 
...
The following is a template for new model. The declaration of `inheritParams` from the `SIS` model in `ramp.xde` works naturally. The namespace from `ramp.xde` is needed, so our Xname function (above) has `ramp.xde::F_X.` 
