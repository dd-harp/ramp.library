...
For the **MY** component, the **BRQS** module implements a behavioral state model for mosquito ecology and infection dynamics with two orgthogonal state spaces. It is a system of ordinary differential equations. The following concise summary is eplained in greater length below: 
...
$$
\begin{array}{rl}
\dot B &= \Lambda \omega + \rho S + (\nu + \theta) \delta Q + \xi R - (f + \zeta_B) B - \Omega_B \cdot B \\
\dot S &= \Lambda (1-\omega)+ (\nu + \theta)(1-\delta) Q + \zeta B  - \rho S - \Omega_S \cdot S \\  
